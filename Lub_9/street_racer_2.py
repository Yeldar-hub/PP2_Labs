...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, SCREEN_WIDTH-40), random.randint(0, 200))
...
while True:
    ...
    coins_text = font_small.render("Points: " + str(COINS), True, BLACK)
    ...
    if pygame.sprite.spritecollideany(P1, super_coins):
        COINS += 3
        for super_coin in super_coins:
            super_coin.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)
    
    #To be run if collision occurs between Player and Enemy
    if pygame.sprite.spritecollideany(P1, enemies):
          pygame.mixer.Sound('crash2.wav').play()
          time.sleep(0.5)
